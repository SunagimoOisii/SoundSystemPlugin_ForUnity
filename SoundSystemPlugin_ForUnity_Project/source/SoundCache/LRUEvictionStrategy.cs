namespace SoundSystem
{
    ...
    /// 最終アクセス時間に基づく削除戦略
    /// </summary>
    internal sealed class LRUEvictionStrategy : IEvictionStrategy
    {
        private readonly float idleTimeThreshold;
        private readonly Dictionary<string, float> lastAccessTime = new();

        public LRUEvictionStrategy(float idleTimeThreshold)
        {
        ...
        {
            if (key != null) lastAccessTime[key] = Time.time;
        }
        ...
        {
            if (key != null) lastAccessTime.Remove(key);
        }
